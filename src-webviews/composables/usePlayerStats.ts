...
export function usePlayerStats() {
    if (!isInit) {
        console.log('REGISTER COMPOSABLE EVENTS!');
        // Player
        WebViewEvents.on(Events.localPlayer.stats.armour, (armour: number) => {
            console.log('armour updated', armour);
            data.value.armour = armour;
        });
        WebViewEvents.on(Events.localPlayer.stats.health, (health: number) => {
            console.log('Update Healt');
